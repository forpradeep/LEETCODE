...
function Signup() {
 ...
 // traverse the form state using useForm hook and fill the form with the resolver using zod schema
  const {
    register, //  contains the register function to register input fields
    handleSubmit, // contains the handleSubmit function to handle form submission
    formState: { errors }, // contains the errors object to access validation errors
  } = useForm({ resolver: zodResolver(signupSchema) }); // Using renamed schema
  ...
  const onSubmit = (data) => {
    dispatch(registerUser(data));  // Dispatch the registerUser action with form data , trigger the async thunk to make the API call and handle the response
  };
