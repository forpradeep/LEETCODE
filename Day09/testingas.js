const json ={ // structure of the problem statement
    ...
    "referenceSolution": [ // Used to verify testcases before saving problem in our DB
        {
            "language": "C++",
...
userSchema.post('findOneAndDelete', async function (doc) {
    if (doc) {
      await mongoose.model('submission').deleteMany({ userId: doc._id }); // Delete all submissions associated with the deleted user
    }
});
...
submissionSchema.index({userId:1,problemId:1}); // search by userId and problemId
...
const submitCodeRateLimiter = async (req, res, next) => {
  const userId = req.result._id;  // Assuming user ID is available in req.result._id after authentication
  const redisKey = `submit_cooldown:${userId}`; // cooldown key for the user

  try {
    // Check if user has a recent submission
    const exists = await redisClient.exists(redisKey); // Check if the cooldown key exists for the user
