...
const SubmissionHistory = ({ problemId }) => {
  ...
  useEffect(() => {
    const fetchSubmissions = async () => { // Fetch submission history for the given problemId
      try {
        setLoading(true); // Set loading state to true before making the API call
        const response = await axiosClient.get(`/problem/submittedProblem/${problemId}`); // Make a GET request to fetch submission history for the given problemId
        setSubmissions(response.data); // Update the submissions state with the fetched data
        setError(null);
      } catch (err) {
  ...
  const formatDate = (dateString) => { // Format the date string into a more readable format
    return new Date(dateString).toLocaleString();
  };

  if (loading) { // Show a loading spinner while fetching data
    return (
  ...
  if (error) { // Show an error message if there was an error fetching data
    return (
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      {submissions.length === 0 ? ( // Show a message if there are no submissions for the problem 
